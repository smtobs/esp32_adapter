...
static struct list_head scan_bss_info_hash[HASH_TABLE_SIZE];
...
void scan_bss_info_entry_init(void)
{
    ...
    {
        INIT_LIST_HEAD(&scan_bss_info_hash[i]);
    }
}

struct bss_info_entry *scan_bss_info_get(u8 *bssid)
{
    ...
    list_for_each_entry(entry, &scan_bss_info_hash[index], list)
    {
...
int scan_bss_info_insert(struct bss_info *new_bss)
{
    u32 index;
    struct bss_info_entry *entry;
    struct bss_info *bss;
    ...
    entry = scan_bss_info_get(new_bss->bssid);
    ...
    if (!entry)
    {
        ...
        list_add(&entry->list, &scan_bss_info_hash[index]);
    }
...
void scan_bss_info_entry_delete(void)
{
    ...
    for (i = 0; i < HASH_TABLE_SIZE; i++)
    {
        if (list_empty(&scan_bss_info_hash[i]))
        {
            continue;
        }

        list_for_each_entry_safe(pos, q, &scan_bss_info_hash[i], list)
        {
            if (pos->bss_info)
            {
